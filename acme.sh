...
if [[ $domainIP = $v4 ]]; then
yellow "当前域名解析到的IPV4：$domainIP" && sleep 1
bash /root/.acme.sh/acme.sh --issue --dns dns_cf -d ${ym} -k ec-256 --server letsencrypt
fi
if [[ $domainIP = $v6 ]]; then
yellow "当前域名解析到的IPV6：$domainIP" && sleep 1
bash /root/.acme.sh/acme.sh --issue --dns dns_cf -d ${ym} -k ec-256 --server letsencrypt --listen-v6
fi
else
dns 
fi
...
if [[ -n $(echo $domainIP | grep nginx) ]]; then
yellow "当前域名解析到的IP：无"
red "域名解析无效，请检查二级域名是否填写正确或稍等几分钟等待解析完成再执行脚本" && rm -rf acme.sh && exit 1
...
dns(){
green "经CF解析，当前为泛域名申请证书模式，目前脚本仅支持Cloudflare的DNS API申请"
readp "请复制Cloudflare的Global API Key:" GAK
export CF_Key="$GAK"
readp "请输入登录Cloudflare的注册邮箱地址:" CFemail
export CF_Email="$CFemail"
...
ab="1.选择standalone独立模式申请证书（需域名）\n2.选择DNS API模式申请证书（需域名、CF注册邮箱、CF Key）\n0.返回上一层\n 请选择："
readp "$ab" cd
case "$cd" in 
...
bash /root/.acme.sh/acme.sh --renew -d ${ym} --force
checktls
else
red "未找到你输入的${ym}域名证书，请自行核实！"
fi
...
yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
green " 1. 首次ACME申请证书（支持standalone模式与DNS API模式，自动识别单域名与泛域名） "
